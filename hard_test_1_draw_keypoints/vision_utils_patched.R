...
draw_keypoints <- function(image,
  ...
  if (is.null(colors)) {
    colors <- grDevices::rainbow(num_instances)
  ...
  for (pose in seq_len(num_instances)) {
    graphics::points(img_kpts[pose,,1], img_kpts[pose,,2],
                     pch = ".", col = colors[[pose]], cex = radius)

    if (!is.null(connectivity)) {
      for (connection in connectivity) {
        start_pt_x <- img_kpts[pose, connection[[1]], 1]
        start_pt_y <- img_kpts[pose, connection[[1]], 2]

        end_pt_x <- img_kpts[pose, connection[[2]], 1]
        end_pt_y <- img_kpts[pose, connection[[2]], 2]

        graphics::lines(
          c(start_pt_x, end_pt_x),
          c(start_pt_y, end_pt_y),
          col = colors[[pose]],
          lwd = width
        )
      }
